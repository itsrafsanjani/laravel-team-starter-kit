...
Always use **Facades** instead of helpers.
Helpers often cause confusion and reduce clarity.
...
Prefer **Form Request Validation** over inline validation within controllers.
This keeps controllers clean and ensures proper separation of concerns.
...
Use the `cn()` helper from `@/lib/utils` for merging class names.
It combines `clsx` and `tailwind-merge` under the hood, ensuring clean and conflict-free class management.
...
Do not use `isset()` in PHP.
Use the **null coalescing operator (`??`)** for a cleaner and more expressive approach.
...
Favor creating **additional controllers** over adding non-resource methods within existing ones.
This keeps responsibilities well-defined and code easier to maintain.
